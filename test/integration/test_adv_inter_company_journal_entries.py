...
def load_je():
    ...
    with open('./test/integration/data/adv_inter_company_journal_entries/' + filename) as oj:
        s = oj.read()
        return json.loads(s)
...
def test_post(nc):
    ...
    res = nc.adv_inter_company_journal_entries.post(je1)
    logger.debug('res = %s', res)
    assert res['externalId'] == je1['externalId'], 'External ID does not match'
    assert res['type'] == 'advInterCompanyJournalEntry', 'Type does not match'

def test_get(nc):
    je1 = load_je()
    data = nc.adv_inter_company_journal_entries.get(externalId=je1['externalId'])
    validate_je(je1, data)

def test_get_all(nc):
    je1 = load_je()
    data = nc.adv_inter_company_journal_entries.get_all_generator()
